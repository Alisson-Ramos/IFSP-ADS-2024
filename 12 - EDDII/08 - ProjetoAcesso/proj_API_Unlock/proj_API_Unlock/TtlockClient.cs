...
namespace proj_API_Unlock
{
    public class TtlockClient
    {
        ...
        public static async Task<string> LoginAsync(HttpClient client, string clientId, string clientSecret, string username, string passwordMd5)
        {
            ...
            using (var response = await client.PostAsync(url, content))
            {
                ...
                if (!response.IsSuccessStatusCode)
                {
                     throw new Exception("HTTP login error " + response.StatusCode + ": " + responseBody);
                }

                var serializer = new JavaScriptSerializer();
                var tokenResp = serializer.Deserialize<TtlockTokenResponse>(responseBody);

                if (tokenResp == null || string.IsNullOrEmpty(tokenResp.access_token))
                {
                    throw new Exception("Falha no login: " + responseBody);
                }
